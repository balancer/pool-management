...
export default class PoolStore {
    ...
    isInvestParamsLoaded(poolAddress, account) {
        ...
        if (
            this.rootStore.tokenStore.hasBalance(poolAddress, account) === false
        ) {
            return false;
        }
    ...
    @action fetchKnownPools = async () => {
        ...
        console.log(
            `Getting pool for factory address ${deployed['kovan'].bFactory}`
        );
        const factory = blockchain.loadObject(
            'BFactory',
            deployed['kovan'].bFactory,
            'BFactory'
        );

        const events = await factory.getPastEvents(LOG_NEW_POOL_EVENT, {
            fromBlock: 0,
            toBlock: 'latest',
        });
        ...
        for (const event of events) {
            poolData[event.returnValues.pool] = {
                manager: event.returnValues.caller,
            };
        }
    ...
    @action fetchInvestParams = async (poolAddress, account) => {
        ...
        try {
            ...
            this.rootStore.tokenStore.setBalanceProperty(
                poolAddress,
                account,
                userBalance
            );
    ...
    @action fetchTokenParams = async poolAddress => {
        ...
        try {
            ...
            for (const key of Object.keys(tokenWeights)) {
                ...
                tokenStore.symbols[key] = deployed['kovan'].tokens.find(
                    x => x.address === key
                ).symbol;
                tokenList.push(key);
            }
        ...
        try {
            ...
            for (const token of tokenWhitelist) {
                if (!tokenWeights[token.address]) {
                    ...
                    await tokenStore.fetchBalanceOf(
                        token.address,
                        defaultAccount
                    );
    ...
    @action rebind = async (
        ...
        try {
            await pool.methods
                .rebind(tokenAddress, tokenBalance, tokenWeight)
                .send();
            await this.fetchTokenParams(poolAddress);
            await this.fetchAllWhitelistedTokenParams(poolAddress);
        } catch (e) {
    ...
    /* 
        Swap Methods - Action
    */
    @action swapExactAmountIn = async (
        poolAddress,
        tokenIn,
        tokenAmountIn,
        tokenOut,
    ...
    @action swapExactAmountOut = async (
        poolAddress,
        tokenIn,
        maxAmountIn,
        tokenOut,
    ...
    */
    previewSwapExactAmountIn = async (
        poolAddress,
        tokenIn,
        tokenAmountIn,
        tokenOut,
        ...
        const bPool = blockchain.loadObject('BPool', poolAddress, 'BPool');
        console.log(
            '[Action] previewSwapExactAmountIn',
            poolAddress,
            tokenIn,
            tokenAmountIn,
            tokenOut,
        ...
        try {
            ...
            const preview = await bPool.methods
                .swapExactAmountIn(
                    tokenIn,
                    tokenAmountIn,
                    tokenOut,
    ...
    previewSwapExactAmountOut = async (
        poolAddress,
        tokenIn,
        maxAmountIn,
        tokenOut,
        ...
        const bPool = blockchain.loadObject('BPool', poolAddress, 'BPool');
        console.log(
            '[Action] previewSwapExactAmountOut',
            poolAddress,
            tokenIn,
            maxAmountIn,
            tokenOut,
    ...
    */
    ...
    @action joinswapExternAmountIn = async (
        ...
        const defaultAccount = blockchain.getDefaultAccount();
        await bPool.methods
            .joinswapExternAmountIn(tokenIn, tokenAmountIn)
            .send();
        await this.fetchTokenParams(poolAddress);
        await this.fetchInvestParams(poolAddress, defaultAccount);
    };
    @action joinswapPoolAmountOut = async (
        poolAddress,
        tokenIn,
        poolAmountOut
    ) => {
        ...
        const defaultAccount = blockchain.getDefaultAccount();
        await bPool.methods
            .joinswapPoolAmountOut(poolAmountOut, tokenIn)
            .send();
        await this.fetchTokenParams(poolAddress);
        await this.fetchInvestParams(poolAddress, defaultAccount);
    };
    ...
    */
    ...
    @action exitswapExternAmountOut = async (
        poolAddress,
        tokenOut,
        tokenAmountOut
    ) => {
        ...
        const defaultAccount = blockchain.getDefaultAccount();
        await bPool.methods
            .exitswapExternAmountOut(tokenOut, tokenAmountOut)
            .send();
        await this.fetchTokenParams(poolAddress);
        await this.fetchInvestParams(poolAddress, defaultAccount);
    };
